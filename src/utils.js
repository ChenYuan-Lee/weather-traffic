const zeroPad = (num) => {
    return ('0'+num).slice(-2);
};
  
export const parseDateTime = (dt) => {
    const year = dt.getFullYear();
    const mth = zeroPad(dt.getMonth()+1);
    const day = zeroPad(dt.getDate());
    const hour = zeroPad(dt.getHours());
    const min = zeroPad(dt.getMinutes());
    const sec = zeroPad(dt.getSeconds());
    return { year, mth, day, hour, min, sec }
};

export const fetchData = async (dateTime, url) => {
    const { year, mth, day, hour, min, sec } = dateTime;
    const dateTimeStr = `${year}-${mth}-${day}T${hour}:${min}:${sec}`;
    const urlWithParams = `${url}?date_time=${dateTimeStr}`;
    const resp = await fetch(urlWithParams, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
    });
    return await resp.json();
};

export const distance = (lat1, lon1, lat2, lon2, unit="K") => {
    if ((lat1 === lat2) && (lon1 === lon2)) {
        return 0;
    }
    const radlat1 = Math.PI * lat1/180;
    const radlat2 = Math.PI * lat2/180;
    const theta = lon1-lon2;
    const radtheta = Math.PI * theta/180;
    let dist = Math.sin(radlat1) * Math.sin(radlat2) + Math.cos(radlat1) * Math.cos(radlat2) * Math.cos(radtheta);
    if (dist > 1) { dist = 1 }
    dist = Math.acos(dist);
    dist = dist * 180/Math.PI;
    dist = dist * 60 * 1.1515;
    if (unit==="K") { dist = dist * 1.609344 }
    if (unit==="N") { dist = dist * 0.8684 }
    return dist;
}